...
notebooks = {
    'multiband_photometry': {
        'file': 'fornax-demo-notebooks/forced_photometry/multiband_photometry.md',
...
def test_notebooks(notebook):
    ...
    with change_dir(f'{notebook_dir}/{nb_path}'):
        comman.run_cmd(f'pip install -r {nb_req}')
        comman.run_cmd(f'jupytext --to py {nb_filename}')
        comman.run_cmd(f'python {py_filename}')
